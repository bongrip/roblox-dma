...
Memory::Memory()
{
	LOG("[-] Loading vmm, ftd3xx & leechcore\n");
...
bool Memory::Init(std::string process_name, bool memMap, bool debug)
{
	...
	reinit:
		...
		if (memMap)
		{
			...
			if (!std::filesystem::exists(path))
				...
				dumped = true;
			LOG("[+] Creating memory map...\n");
			if (!dumped)
			{
				LOG("[-] Failed to create memory map!\n");
				LOG("[-] NOTICE: Using no memory map, this may result in slower reads.\n");
			}
			else
			{
				LOG("[+] Created memory map!\n");
		...
		if (!this->vHandle)
		{
			...
			{
				memMap = false;
				LOG("[-] Failed to open device with Memory Map, Please try again :(\n");
				goto reinit;
			}
		...
		//LOG("[+] FPGA: %llu\n", FPGA_ID);
		//LOG("[+] Device: %llu\n", DEVICE_ID);
		LOG("[+] Success! DMA device has been opened\n");
		...
		LOG("[+] Device ::init() called but DMA is already loaded?\n");

	if (PROCESS_INITIALIZED)
	{
		LOG("[+] Device ::init(...) called buut DMA is already loaded & process is located?\n");
		return true;
	}
	...
	{
		LOG("[-] Failed to convert name to PID.\n");
		return false;
	}
	this->current_process.process_name = process_name;
	if (!TargetProcess.FixCr3())
		std::cout << "[-] Failed to fix CR3, memory is shuffled!" << std::endl;
	else
		//std::cout << "[+] Fixed CR3" << std::endl;
...
bool Memory::Write(uintptr_t address, void* buffer, size_t size) const
{
	...
	{
		LOG("[-] Memory write operation fail @ 0x%p\n", address);
		return false;
	}
	return true;
}
...
bool Memory::Read(uintptr_t address, void* buffer, size_t size) const
{
	...
	{
		LOG("[-] Memory read operation fail @ 0x%p\n", address);
		return false;
	}
	return true;
}
